...
public class FireballBehavior02 : Behavior
{
    ...
    {
        Target = new Target { TargetType = Enums.TargetType.All, TargetDirection = Enums.TargetDirection.Enemy, Quantity = 1 };
        MinMaxProperty = MinMaxProperty.New(Enums.PropertyName.BehaviorValue, -20, -15);
        PropertyName = Enums.PropertyName.Life;
        Properties.Add(FlatProperty.New(Enums.PropertyName.BehaviorValue, 1));
        Costs.Add(Cost.New(-5, Enums.CostType.Percentage, Enums.PropertyName.Life));
    }
    ...
    public override void Execute(Unit unitInAction, List<Unit> ownUnits, List<Unit> enemyUnits, HashSet<int> targetIDs, Random random)
    {
        ...
        for (var i = 0; i < 2; i++)
        {
            foreach (var unitTarget in unitTargets)
            {
                ...
                if (BehaviorType == Enums.BehaviorType.Damage)
                {
